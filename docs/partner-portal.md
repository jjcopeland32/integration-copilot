...
## 1. Purpose
The existing Integration Copilot UI targets *our* customer (the enterprise API owner). They can ingest partner specs, spin up mocks, run golden tests, and assess readiness. We now need the companion portal for the *partner* (e.g., Lowe’s integrating SYF’s payments API). This portal should help the partner team:

1. Understand exactly what SYF expects (latest blueprint + plan).
2. Upload their own sandbox credentials/spec changes.
3. Run the same golden tests/mocks themselves, see evidence, and share logs with SYF.
...
| SYF CSM (read-only) | Observe progress without logging into internal cockpit | Shared Reports, Activity Feed |

### Core Journey
1. **Invite** – SYF invites a partner (email, token). Partner self-onboards and lands on Partner Dashboard.
2. **Spec Sync** – Partner either confirms the spec SYF shared or uploads their own variant (diff surfaced back to SYF).
3. **Mock/Test Runs** – Partner can trigger the same automation CTA (generate mocks/tests) scoped to their sandbox credentials.
4. **Evidence & Plan** – Partner sees their specific plan phases (subset of SYF’s) and uploads evidence per exit criterion.
5. **AI Guidance** – Partner can ask questions referencing latest tests/traces/evidence.
6. **Report Sharing** – Partner can request readiness review; SYF approves via internal cockpit.
...
2. **PartnerProject** linking to SYF Project (one SYF project ↔ multiple partner projects).
...
| Specs | View latest spec, upload partner variant, diff viewer | Diffs notify SYF via internal cockpit |
...
2. **Auth**
   - Invitation flow: SYF triggers invite → email link (`token`).
...
1. **Evidence Notifications** – when partner uploads evidence, notify SYF inside internal cockpit + Slack/Jira hooks.
2. **Report Preview** – partner can request readiness; portal shows status (“Under Review”, “Approved”).
3. **Observability** – partner-level activity feed for CSM visibility.
